...
int mersenne(int power) { return (Pow(2, power) - 1); }
...
// function to find the lucass
int lucas(int index, int element1, int element2) {
  if (index == 0) { // luc[index==0] = 2
    return 2;
  }

  else if (index == 1) { // luc[index==1] = 1
    return 1;
  }

  else { // lucas[index>1] = (index-1) + (index-2)
    return element1 + element2;
  }
}
...
char *buildstring(char str[], int from_decimal, int to_base) {
  ...
  while (from_decimal > 0) {
    ...
    // otherwise its hexa
      ...
      // fill string index with asci lowercase letter
...
void primes(int length, BitVector *v) {
  ...
  // arrays to hold luc and fib numbers
  // declare and initalize values to 0
  int fib[20] = {0};
  int luc[20] = {0};

  // populate the fib and lucas arrays
  for (int i = 0; i < 20; i++) {
    fib[i] = fibonacci(i, fib[i - 1], fib[i - 2]);
    luc[i] = lucas(i, luc[i - 1], luc[i - 2]);
  }
