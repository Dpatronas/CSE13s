...
void recursive_tower(int n, char initial, char goal, char extra) {
  ...
  if (n == 1) {
    // move the top into the desired peg based on the current function call
    // configuration
...
void display(Stack *s1, Stack *s2) {
  printf("\nMove disk %d from peg %c to peg %c", s_peek(s1), s1->name,
         s2->name);
}

// compares values at the top of each peg
// if no disk exists in peg the value is set to n+1 (for precedence in
// determining move)  decides which direction the disk will move
    ...
    for (int i = 0; i < num_moves; i++) {
      ...
      case 0:
        ...
        // otherwise (extra->source)
          ...
          display(extra, source);
          //(goal->source)push the value that is popped from goal into the
          //source
          s_push(source, s_pop(extra));
        }
        break;
  ...
  while ((c = getopt(argc, argv, OPTIONS)) != -1) {
    ...
    case 'n':
      ...
      if (atoi(input_disks) > 0) {
        disks = atoi(input_disks); // convert the char to int and set static
                                   // variable to it
        break;
      }
